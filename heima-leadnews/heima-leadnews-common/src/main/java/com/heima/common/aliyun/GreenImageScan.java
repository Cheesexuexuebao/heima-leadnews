...
public class GreenImageScan {
    ...
    public Map imageScan(List<byte[]> imageList) {
        Random random = new Random();
        Map<String, String> resultMap = new HashMap<>();

        // 随机决定是否通过检测 (70%概率通过)
        boolean isPass = random.nextDouble() > 0.3;
        ...
        // 随机选择违规类型
        List<String> violationLabels = Arrays.asList(
                "porn",          // 色情
                "terrorism",    // 暴恐
                "ad",           // 广告
                "qrcode",       // 二维码
        ...
        // 随机选择一个违规标签
        String label = violationLabels.get(random.nextInt(violationLabels.size()));
